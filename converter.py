...
for pdf_path in pdf_folder.glob("*.pdf"):
    ...
    for rows in daily_rows:
        for i, info in enumerate(rows):
            if info[0] == 'S': # one column contains title for summary and data, which we dont need to convert into float, but we do want to remove the text so the dates can easily be compared to the other table
                rows[i] = rows[i].replace("Summary for ", "")
                continue
            rows[i] = float(rows[i])
